...
(function () {
  ...
  let streamThrottleTimeout = null;

  socket.on("stream", (data) => {
    if (streamDiv) {
      pendingStreamContent += data.content;

      // Throttle DOM updates to once every 100ms to prevent O(N^2) markdown rendering overhead
      if (!streamThrottleTimeout) {
        streamThrottleTimeout = setTimeout(() => {
          const contentEl = streamDiv.querySelector(".msg-content");
          const dots = contentEl.querySelector(".typing-indicator");
          if (dots) dots.remove();

          currentStreamRawText += pendingStreamContent;
          contentEl.innerHTML = formatMarkdown(currentStreamRawText);
  ...
  socket.on("stream_end", (data) => {
    if (streamDiv) {
      if (streamThrottleTimeout) clearTimeout(streamThrottleTimeout);
      const contentEl = streamDiv.querySelector(".msg-content");
