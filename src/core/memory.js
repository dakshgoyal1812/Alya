...
/**
 * Helper to construct the standardized JSON payload for conversations.
 * Eliminates duplication and guarantees structure consistency.
 */
...
/**
 * Save a conversation to disk asynchronously to prevent event loop blocking during chat.
 * Performance Optimization: Shifts CPU-heavy serialization and file write off the main thread where possible.
 */
...
/**
 * Save a conversation to disk synchronously. Used on exit or direct cleanups to guarantee persistence.
 */
...
export function addMessage(platform, channelId, role, content) {
  ...
  // Save to disk every 5 messages - Optimized: use non-blocking async save
