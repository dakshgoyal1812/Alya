...
export function flushAll() {
  for (const [key] of conversations) {
    // Performance Optimization: Using indexOf(":") + substring() instead of split(":")
    // to prevent array allocations and to properly handle platform channel IDs containing multiple colons.
...
export function getStats() {
  ...
  for (const [key, messages] of conversations) {
    ...
    // Performance Optimization: Using indexOf(":") + substring() instead of split(":")
    // to avoid heap allocation overhead and avoid truncating channel IDs containing colons.
    const colonIndex = key.indexOf(":");
