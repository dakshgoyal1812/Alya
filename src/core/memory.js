...
/**
 * Helper to structure the conversation JSON payload
 */
...
/**
 * Save a conversation to disk asynchronously (non-blocking)
 * Optimized for frequent chat updates
 */
async function saveToDiskAsync(platform, channelId) {
  ...
  try {
    await fs.promises.writeFile(filePath, serializeConversation(platform, channelId, messages));
  } catch (error) {
...
export function addMessage(platform, channelId, role, content) {
  ...
  // Save to disk every 5 messages (using non-blocking async method)
