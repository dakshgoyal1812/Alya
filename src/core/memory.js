...
/**
 * Save a conversation to disk synchronously (used for exiting or explicit flushes)
 */
...
export function addMessage(platform, channelId, role, content) {
  ...
  // Save to disk asynchronously every 5 messages to avoid blocking the event loop
...
/**
 * Flush all conversations to disk synchronously on exit/shutdown
 */
...
export function getStats() {
  ...
  for (const [key, messages] of conversations) {
    ...
    const colonIndex = key.indexOf(":");
    const platform = colonIndex !== -1 ? key.slice(0, colonIndex) : key;
