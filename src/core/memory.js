...
/**
 * Tracks unsaved message count per conversation for periodic flushing
 */
const unsavedCounts = new Map();
...
function saveToDisk(platform, channelId) {
  ...
  try {
    ...
    // Reset unsaved counter on success
    unsavedCounts.set(key, 0);
  } catch (error) {
...
export function addMessage(platform, channelId, role, content) {
  ...
  const unsaved = (unsavedCounts.get(key) || 0) + 1;
  unsavedCounts.set(key, unsaved);

  // Save to disk every 5 new messages to reduce I/O overhead
  if (unsaved >= 5) {
...
export function clearHistory(platform, channelId) {
  ...
  unsavedCounts.set(key, 0); // Reset counter
  saveToDisk(platform, channelId);
}

/**
 * Flush all conversations with pending changes to disk
 */
export function flushAll() {
  for (const [key, unsaved] of unsavedCounts) {
    if (unsaved > 0) {
