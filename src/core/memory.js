...
/**
 * BOLT: Tracks how many unsaved messages are in each conversation
 */
...
export function addMessage(platform, channelId, role, content) {
  ...
  // BOLT OPTIMIZATION: Use unsavedCounts to trigger save every 5 messages
  // This avoids redundant saves after MAX_HISTORY is reached.
  const count = (unsavedCounts.get(key) || 0) + 1;
  if (count >= 5) {
    saveToDisk(platform, channelId);
    unsavedCounts.set(key, 0);
  } else {
    unsavedCounts.set(key, count);
  }
}
