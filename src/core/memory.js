...
/**
 * Helper to serialize conversation payload consistently
 */
...
/**
 * Save a conversation to disk synchronously (used on process exit)
 */
...
export function addMessage(platform, channelId, role, content) {
  ...
  if (history.length % 5 === 0) {
    saveToDiskAsync(platform, channelId).catch(err => {
      console.error(`Error in periodic save for ${platform}:${channelId}:`, err.message);
    });
  }
}
...
export function clearHistory(platform, channelId) {
  ...
  saveToDiskAsync(platform, channelId).catch(err => {
    console.error(`Error in clearing save for ${platform}:${channelId}:`, err.message);
  });
}
...
export function flushAll() {
  for (const [key] of conversations) {
    ...
    const channelId = colonIndex !== -1 ? key.substring(colonIndex + 1) : "";
    saveToDisk(platform, channelId);
  }
}

/**
 * Get conversation statistics with allocation-free key parsing
 */
