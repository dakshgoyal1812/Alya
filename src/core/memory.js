...
/**
 * Tracks unsaved message counts per conversation to throttle disk writes
 */
...
function saveToDisk(platform, channelId) {
  ...
  try {
    ...
    // Reset unsaved count on successful write
...
export function addMessage(platform, channelId, role, content) {
  ...
  // Increment unsaved message counter
  const unsavedCount = (unsavedCounts.get(key) || 0) + 1;
  unsavedCounts.set(key, unsavedCount);

  // Save to disk every 5 messages (optimized to avoid redundant writes when history is full)
  if (unsavedCount >= 5) {
