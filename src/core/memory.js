...
/**
 * Serialize conversation to JSON format
 */
...
/**
 * Save a conversation to disk asynchronously to prevent event loop blocking, using a queue to ensure sequential writes
 */
async function saveToDiskAsync(platform, channelId) {
  const key = getKey(platform, channelId);

  let queue = writeQueues.get(key) || Promise.resolve();
...
export function flushAll() {
  for (const [key] of conversations) {
    ...
    const channelId = index !== -1 ? key.substring(index + 1) : "";
    saveToDisk(platform, channelId);
  }
}
