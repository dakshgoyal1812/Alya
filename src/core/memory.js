...
/**
 * Save a conversation to disk (Asynchronous to prevent event loop blocking)
 */
async function saveToDisk(platform, channelId) {
...
function saveToDiskSync(platform, channelId) {
  ...
  try {
    ...
    console.error(`Error saving conversation ${platform}:${channelId} (Sync):`, error.message);
  }
}
...
/**
 * Flush all conversations to disk (Synchronous)
 */
export function flushAll() {
  for (const [key] of conversations) {
    const parts = key.split(":");
    const platform = parts[0];
    const channelId = parts.slice(1).join(":"); // Handle IDs that might contain colons
