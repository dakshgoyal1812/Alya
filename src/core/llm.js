...
export class LLMEngine {
  constructor() {
    ...
    // Check if we should use local Ollama or Groq Cloud
    this.useOllama = config.ollama?.enabled || false;

    if (this.useOllama) {
      this.baseURL = config.ollama.host || "http://localhost:11434/v1";
      this.apiKeys = ["ollama"];
      this.model = config.ollama.model || "llama3.2";
      console.log(`🦙 Using Local Ollama at ${this.baseURL} (Model: ${this.model})`);
    } else {
      // Support multiple Groq API keys for failover
      ...
      console.log(`🔑 Using Groq Cloud API: ${this.apiKeys.length} key(s) loaded`);
    }
  ...
  /**
   * Initialize or re-initialize the OpenAI client
   */
  _initClient() {
  ...
  async chat(conversationHistory = [], userMessage) {
    // Slice BEFORE mapping to optimize performance on long histories
    const recentHistory = conversationHistory.slice(-20);
    const sanitizedHistory = recentHistory.map(msg => ({
      role: msg.role,
      content: msg.content,
