...
// Hoisted regex instances to avoid re-compiling RegExps on every response/chunk cleanup
...
function cleanResponse(text) {
  if (!text) return "";
  // Fast path: skip regex processing if no '<' character is present
  ...
  clean = clean.replace(LLAMA_TOKEN_REGEX, "");
  return clean.trim();
}
