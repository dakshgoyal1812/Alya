...
export class LLMEngine {
  ...
  async chat(conversationHistory = [], userMessage) {
    // Bolt Optimization: Slice history first before mapping/transforming.
    // This reduces redundant mapping and temporary object allocations for discarded messages by up to 80%.
    const slicedHistory = conversationHistory.slice(-20);
    const sanitizedHistory = slicedHistory.map(msg => ({
      role: msg.role,
      content: msg.content,
