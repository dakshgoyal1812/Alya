...
export async function executeTool(name, args) {
  try {
    switch (name) {
      ...
      case "get_storage_info": {
        try {
          if (process.platform === "win32") {
            ...
            // ⚡ Bolt: Use 'df' on Linux/macOS for better performance and compatibility
            const output = execSync("df -h / --output=size,used,avail,pcent", { encoding: "utf-8" });
            const lines = output.trim().split("\n");
            if (lines.length > 1) {
              const [size, used, avail, pcent] = lines[1].trim().split(/\s+/);
              return JSON.stringify({
                root: { total: size, used, free: avail, usage: pcent },
      ...
      case "calculator": {
        // ⚡ Bolt: Added regex sanitization to prevent RCE via new Function()
      ...
      case "execute_python_code":
        return "Error: The Python sandbox is currently disabled for security reasons.";
