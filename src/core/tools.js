  ...
  try {
    switch (name) {
      ...
      case "get_storage_info": {
        try {
          if (os.platform() === "win32") {
            ...
            // Unix/Linux/macOS
            const output = execSync("df -h --output=source,size,used,avail,pcent /", { encoding: "utf-8" });
            const lines = output.trim().split("\n").filter(l => l.trim());
            const drives = [];
            if (lines.length >= 2) {
          ...
          return JSON.stringify({
            note: `Detailed storage info unavailable (${e.message}), showing memory instead.`,
      ...
      case "search_web": {
        ...
        const searchResults = await search(args.query, { safeSearch: SafeSearchType.STRICT });
