    ...
    switch (name) {
          ...
          if (os.platform() === "win32") {
            ...
            for (let i = 1; i < lines.length; i++) {
              ...
              if (parts.length >= 6) {
                ...
                const mount = parts[parts.length - 1] || parts[5];
                if (totalKB > 0 && (mount === "/" || filesystem.startsWith("/dev/"))) {
                  drives.push({
                    drive: mount,
      ...
      case "calculator":
        ...
        if (!/^[0-9+\-*/().\s]+$/.test(args.expression)) {
          return "Error: Invalid math expression. Only digits, basic arithmetic operators, parentheses, and spaces are allowed.";
        }
        try {
      ...
      case "read_website": {
        try {
          ...
          // Block local, loopback, private, broadcast IPs/hostnames
          const blockedHosts = ["localhost", "127.0.0.1", "0.0.0.0", "::1", "::"];
          if (blockedHosts.includes(hostname) || hostname.endsWith(".local") || hostname.endsWith(".internal")) {
            return "Error: Access to private/local network resources is restricted (SSRF Protection).";
          }
          ...
          if (
            ...
            return "Error: Access to private/local IP addresses is restricted (SSRF Protection).";
          }
      ...
      case "read_pdf": {
        try {
          if (!args.absolutePath || typeof args.absolutePath !== "string") {
            return "Error: Path is required.";
          }
          if (args.absolutePath.includes("..")) {
            return "Error: Relative path traversal ('..') is not permitted.";
          }
          const resolvedPath = path.resolve(args.absolutePath);
          const normalizedPath = resolvedPath.replace(/\\/g, "/");
          const lowerPath = normalizedPath.toLowerCase();

          const forbiddenPaths = ["/etc/", "/var/", "system32", ".git", "config/"];
          for (const forbidden of forbiddenPaths) {
            if (lowerPath.includes(forbidden)) {
              return `Error: Access to path containing '${forbidden}' is restricted.`;
            }
          }
      ...
      case "execute_python_code": {
        return "Security Notice: Python execution tool is disabled on host environments to prevent arbitrary code execution.";
      }
