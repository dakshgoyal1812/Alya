  ...
  try {
    switch (name) {
      ...
      case "get_storage_info": {
        try {
          if (process.platform === "win32") {
            ...
            return JSON.stringify({ platform: "windows", drives, deviceName: os.hostname() });
          } else {
            // Linux/macOS
      ...
      case "calculator": {
        ...
        if (!/^[0-9+\-*/().\s]+$/.test(expr)) {
          return "Error: Invalid characters in mathematical expression. Only basic math operations are allowed.";
        }
      ...
      case "execute_python_code": {
        // Disabled for security reasons to mitigate arbitrary remote code execution on the host system.
        return "Error: Python code execution is disabled in this environment for security reasons.";
      }
