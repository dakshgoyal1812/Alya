    ...
    switch (name) {
      ...
      case "calculator":
        ...
        if (!/^[0-9+\-*/().\s]+$/.test(args.expression)) {
          return "Error: Invalid math expression. Only basic arithmetic numbers and operators (+, -, *, /, ()) are allowed.";
        }
      ...
      case "read_website": {
        try {
          ...
          const isPrivateIp = /^10\.|^192\.168\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.|^127\./.test(hostname);
          if (forbiddenHosts.includes(hostname) || isPrivateIp || hostname.endsWith(".local") || hostname.endsWith(".internal")) {
            return "Error: Access to private/internal loopback addresses and metadata endpoints is restricted for security.";
          }
      ...
      case "read_pdf": {
        try {
          if (args.absolutePath.includes("..")) {
            return "Error: Path traversal sequences ('..') are blocked for security.";
          }
          const resolvedPath = path.resolve(args.absolutePath);
          const normalizedPath = resolvedPath.replace(/\\/g, "/");
          const restrictedPaths = ["/etc/", "/var/", "system32", ".git", "config/"];
          if (restrictedPaths.some(p => normalizedPath.toLowerCase().includes(p))) {
            return `Error: Access to path '${args.absolutePath}' is restricted.`;
          }
      ...
      case "execute_python_code": {
        return "Security Warning: The 'execute_python_code' tool is disabled on host environments to prevent arbitrary shell command execution.";
      }
