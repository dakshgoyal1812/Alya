...
export async function executeTool(name, args) {
  try {
    switch (name) {
      ...
      case "calculator": {
        // Evaluate math expression using only allowed characters (numbers and basic operators)
        const sanitizedExpression = args.expression.replace(/[^0-9+\-*/().\s]/g, "");
        if (!sanitizedExpression || sanitizedExpression !== args.expression.trim()) {
          return "Error: Mathematical expression contains forbidden characters.";
        }
        try {
          // Use Function constructor with restricted scope for simple arithmetic
          return String(new Function(`return (${sanitizedExpression})`)());
        } catch (e) {
      ...
      case "read_pdf": {
        try {
          // Prevent arbitrary file access: only allow files in data/ or home dir if safe
          // For now, let's restrict to process.cwd() / data
          const resolvedPath = path.resolve(args.absolutePath);
          if (!resolvedPath.startsWith(path.join(process.cwd(), "data"))) {
             return "Error: Access to this file path is restricted for security reasons.";
          }
      ...
      case "execute_python_code": {
        return "System Note: The Python sandbox is currently disabled for security maintenance. Please use the calculator tool for math or perform calculations manually.";
      }
