...
export const availableTools = [
  ...
  },
  {
    ...
    function: {
      name: "read_pdf",
      description: "Read text from a PDF file located in the Alya 'data' directory. Provide only the filename (e.g., 'report.pdf').",
      parameters: {
        type: "object",
        properties: {
          filename: { type: "string", description: "The name of the PDF file in the data folder." }
        },
        required: ["filename"]
      }
    }
...
export async function executeTool(name, args) {
  try {
    switch (name) {
      ...
      case "calculator":
        // Strict regex-based math validation to prevent RCE
      ...
      case "read_pdf": {
        try {
          ...
          const pdfPath = path.join(process.cwd(), "data", safeFilename);
          if (!fs.existsSync(pdfPath)) return `File not found in data directory: ${safeFilename}`;
      ...
      case "screenshot_website": {
        try {
          // SSRF Protection: Ensure URL starts with http/https
