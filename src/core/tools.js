    ...
    switch (name) {
            ...
            if (lines.length >= 2) {
              ...
              return JSON.stringify({
                drives: [{
                  ...
                  usagePercent: parts[4] || (Math.round((usedKB / totalKB) * 100) + "%")
                }],
      ...
      case "read_website": {
        try {
          ...
          if (blockedHosts.includes(hostname) || hostname.endsWith(".local") || hostname.endsWith(".internal")) {
            return "Error: Access to internal or loopback network addresses is restricted.";
          }
          if (/^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|169\.254\.)/.test(hostname)) {
            return "Error: Access to private IP ranges is restricted.";
          }
      ...
      case "read_pdf": {
        try {
          ...
          const resolved = path.resolve(args.absolutePath).replace(/\\/g, "/");
          const blockedPaths = ["/etc", "/var", "/usr", "/sys", "/proc", "system32", "windows"];
          if (blockedPaths.some(p => resolved.toLowerCase().includes(p))) {
            return "Error: Access to sensitive system directories is restricted.";
          }
      ...
      case "execute_python_code": {
        return "Error: Python code execution tool is disabled for security reasons to prevent arbitrary system command execution.";
      }
