    ...
    switch (name) {
      ...
      case "calculator":
        if (!/^[0-9+\-*/().\s]+$/.test(args.expression)) {
          return "Error: Invalid math expression. Only digits and basic arithmetic operators are allowed.";
        }
      ...
      case "read_website": {
        try {
          ...
          if (
            ...
            return "Error: Accessing local or private network addresses is restricted for security.";
          }
          const response = await fetch(args.url, { redirect: "manual" });
          if (!response.ok && response.status !== 200) return `Error fetching URL: ${response.status} ${response.statusText}`;
      ...
      case "read_pdf": {
        try {
          if (args.absolutePath.includes("..")) {
            return "Error: Relative path traversal sequences ('..') are blocked for security.";
          }
          const normalizedPath = path.resolve(args.absolutePath).split(path.sep).join("/");
          const restrictedPaths = ["/etc/", "/var/", "/sys/", "/proc/", "/dev/", "system32", "windows"];
          if (restrictedPaths.some(p => normalizedPath.toLowerCase().includes(p))) {
            return "Error: Access to system directories is restricted for security.";
          }
